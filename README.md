...
|---|---|---|
| **Companion inference** | **Default (operator 2026-09-16): Qwen3.5‑2B** `Qwen3.5-2B-Q4_K_M.gguf` (`unsloth` GGUF) — 2B is the smallest reasonable companion tier (DOM access + tool calling + lore decisions). **Atomic Bots catalogue** (offered model list): Qwen3.5‑2B · LFM2.5‑2.6B GGUF (`LiquidAI`; **stability probe required** — operator-recalled crash issues) · Bonsai‑27B / Ternary‑Bonsai‑27B 1‑bit GGUF (`prism-ml`; smart tier — slow load, offered where GPU-fit is verified, Android precondition). Qwen3.5‑0.8B is **below the companion floor** (debug row only). Native llama.cpp; local PWA uses wllama/WASM. **Partial:** model-to-app integration + mirror publication remain. | **Default selected 2026-09-14 (operator):** OpenRouter free tier (`openrouter/free` + config-level fallback list) with rate-limit-aware dynamic backoff; free access is trial-limited. Paid provider for unlocked/metered use **not selected**. **Not implemented:** no hosted adapter exists yet. |
...
| **Quantization / capability** | Atomic chat-turboquant: Q4_K_M chat weights + q8_0 K/V cache (catalogue default). Higher-compressed tiers available at the same source and llama.cpp-standard: `UD-Q3_K_XL` / `IQ4_XS` (catalogue compressed tier; engine-capability probe pending). The `q4r8` recursor remains **unsupported** by the pinned engines — re-pin gated on D16 (four-target matrix). | Server model precision/context depend on the selected provider. **Not selected**; no equivalence claim with the local runtime. |
