...
Recreated 2026-09-09 from `DOCS/ARCHITECTURE.md` (2026-09-09 reconciled edition). Supersedes
the 2026-09-04 translation, which is preserved beside this file as
`CHECKLIST.md.bak.20260909_163202` (additive per I3 — copy, never move). Every task is
**atomic**, **idempotent** (re-running after completion changes nothing and exits 0),
**order-independent** (any task may run before or after any other, in parallel, with four
...
- **Ownership closure:** every Verify command operates only on files in its own task's
  **Owns** — including the test files the Verify runs. Entities are denoted explicitly in
  each block, translated 1:1 from the exhaustive enumeration in `DOCS/ARCHITECTURE.md`,
  which is the source of truth. A task that seems to need a file outside its Owns is a
  checklist defect, not an improvisation license.
...
- Verifies are scoped to owned files (unit-level, local fixtures) so they pass regardless of
  other tasks' state. The ⛓ integrator tasks (I.1–I.4) are regenerators, not coordinators —
  they rebuild registries from whatever exists when they run.
...
- [ ] **T0.2 — apps/local frontend scaffold.**
  ...
  **Reads:** T0.1, T0.8. **Spec:** Vite + React 19 entry that renders `<NatallyApp/>`;
  ...
  `tsconfig.standalone.json` typechecks this task's files alone (router import excluded).
  ...
  **Accept:** `built standalone entry OK` (tsc exits 0). The full-app `vite build` is
  exercised later by I.1/I.4 once the router registry exists — not part of this Accept
  (order-independence).
...
- [ ] **T0.4 — Token mirror test (design-tokens contract verification).**
  ...
  **Reads:** T0.8. **Spec:** test parses `tokens.css` `@theme` block and asserts the 36
  frozen variables: 10 colour vars (STATE-LEDGER 2:3–2:12) + 12 zodiac vars (2:13–2:24) +
  4 radius (2:25–2:28) + 6 space (2:29–2:34) + 1 stroke (2:35) + 3 size (2:36–2:38); every
...
- [ ] **T0.5 — Ephemeris contracts.**
  ...
  **Spec:** verbatim from ARCHITECTURE §6: `EphemerisEngine` (id, init(cfg), position, cusps,
  ...
  `HouseSystem` = the pinned 12, each identical to its `HouseSystems` code of
  `sweph-wasm@2.6.9` (ARCHITECTURE §6 closed set; type surface at
  `DOCS/sdk/sweph-wasm/index.d.ts:2423`): `'P'` Placidus, `'K'` Koch, `'O'` Porphyry,
  `'R'` Regiomontanus, `'C'` Campanus, `'A'` Equal (Asc), `'V'` Vehlow Equal,
  `'W'` Whole Sign, `'T'` Topocentric (Polich/Page), `'X'` Meridian (axial rotation),
  `'B'` Alcabitius, `'U'` Krusinski-Pisa-Goelzer; `OrbTable` (conjunction 8,
...
- [ ] **T0.8 — Design tokens package.**
  ...
  **Spec:** `tokens.css` = Tailwind 4 `@theme` block mirroring TOKENS.md one-to-one (colour,
  ...
  --check` **Accept:** `tokens.css in sync with TOKENS.md (36 vars)`.
...
- [ ] **T0.10 — Config loader.**
  ...
  **Spec:** typed `loadConfig(env)` reading the `.env.example` variables verbatim
  ...
  app URL, `VITE_LICENSE_PUBKEY`, lore flags + embed dim (384 default). This task owns
  `.env.example` whole: add the `VITE_LICENSE_PUBKEY=` line (comment: offline license
  verify — B.3 reads it) and annotate `VITE_TRIAL_MODEL` as required non-empty at build
  (a blank value makes the config loader throw at boot — fail-loud by design). Export
  `RuntimeConfig` frozen object + `paymentsAvailable()` derived list. zod-validated;
  throws with field names on bad env.
...
- [ ] **P.1 — sweph-wasm backend.**
  ...
  **Reads:** T0.5, `DOCS/sdk/sweph-wasm/` (vendored 2.6.9 snapshot). **Spec:** implement
  ...
  `init` loads semiset ephemeris (lazy mount of the bundled `dist/ephe/seas_*.se1` files;
  document the exact files in a `tables.ts` table map); `position` via `swe_calc_ut`
  (speed flags on); `cusps` via `swe_houses_ex` for each of the 12 pinned HouseSystems
  (codes per ARCHITECTURE §6 — they are the engine's own `HouseSystems` codes verbatim);
  time-unknown input never reaches here (P.2 guards). Conformance fixtures: embed 24 cases
  (2 dates × 12 bodies: Gregorian 1990-05-02 14:32 UT and 2026-09-04 00:00 UT; place
  55.60N 13.00E) with expected lon values generated from Swiss Ephemeris reference tables
  (operator supplies at review; until then compute-and-freeze via the same engine is
  FORBIDDEN — instead the test asserts **invariants**: positions in [0,360), |speed| bounds
  per body, cusps ascending modulo, asc/mc consistency `|normalize(mc-asc)| ≤ 180`). When
  operator supplies reference numbers, tighten tol 0.01°.
...
- [ ] **L.5 — Write-every-turn pipeline.**
  ...
  **Spec:** `LorePipeline.consume(turn)` → embed (L.2) → extract/merge (L.3) → batched
  ...
  `{turns, nodes, edges}` for Settings (UI renders `[turns · nodes]`, ARCHITECTURE §8.4);
  `deleteAll()` = rows + vectors real deletion.
...
- [ ] **B.3 — License token verify + storage.**
  ...
  **Reads:** `.env.example` `VITE_LICENSE_PUBKEY` (line owned by T0.10).
  **Spec:** compact token = base64url(header).payload.Ed25519-sig (COSE-esque, no external
  ...
  native); public key baked from config `VITE_LICENSE_PUBKEY`; verify offline: sig, iss,
  sub match appUserId, exp null-or-future; deny-list: signed DenyList fetched when online
  (bridge), cached, `revokedJti` blocks only listed jti; storage: keychain (native command
  via registry macro) / IndexedDB WebCrypto-wrapped (web). Tests: generate test keypair,
  forge + reject tests.
...
- [ ] **B.5a — Generic hosted-redirect adapter + bridge client.**
  **Owns:** `packages/billing/src/adapters/hosted.ts`, `packages/billing/src/bridge-client.ts`,
  `packages/billing/src/adapters/guards.ts`, tests. **Spec:** for
  stripe/polar/lemonsqueezy/paypal/square: `available()` = config URL present;
  `checkout(offering)` → `{kind:'redirect', url: <configured checkout URL> +
  ?appUserId=<id>&offering=<id>}` opened via opener injection (Tauri shell-open /
  window.open); then poll bridge `POST /verify {appUserId}` (2s backoff, 15 min cap,
  offline-tolerant); `restore()` → `GET /verify?appUserId=` → token|null. SSRF guard:
  reject non-https, loopback/private/reserved hosts (owns `guards.ts`).
  **Verify:** `pnpm vitest run packages/billing/tests/adapters-hosted.test.ts` (no network:
  ...
  in-process routing function — the injection is a parameter, not a mock)
...
- [ ] **C.3 — Tool suite (DOM r/w).**
  ...
  **Spec (§7.3):** `dom.read` → accessibility-style snapshot from the live DOM (query from
  ...
  tools module imports no network surface; a test greps the file for `fetch(`,
  `XMLHttpRequest`, `new WebSocket`, `window.open(` and fails on any hit.
...
- [ ] **C.4 — Companion event bus.**
  ...
  **Spec:** typed pub/sub for the CompanionEvent union (token, turn, chart-computed,
  envelope, error); `subscribe(type, fn)` unsubscribe handle; the exported `StageSignal`
  union = `CompanionEvent ∪ {engine-load(fraction), model-presence(bool), composer-focus(bool)}`
  (ARCHITECTURE §10 — asleep/waking/listening ride capability signals, not bus events);
  replay-buffered `stageState$` derived reducer over StageSignal: Thinking on request-sent
  (no tokens yet — STATES.md), Speaking while the envelope plays, Delighted on
  chart-computed/unlock, Error on engine failure, Asleep from model-presence=false, Waking
  from engine-load progress — pure reducer exported for U.9 tests.
...
- [ ] **U.1 — Shell, router contract, primitives.**
  ...
  **Spec:** primitives mirror Components page 1:1 (STATE-LEDGER ids in doc comments; the
  STATE-LEDGER is canonical for component node-ids — DESIGN.md's Stage row is stale pending
  the operator re-clearance, see STATE-LEDGER `pendingFixes`); glyph set loaded from
  `LIBS/UI/FIGMA/glyphs/natally-glyphs.svg` (owns `apps/local/src/assets/glyphs/` copy);
  router: hash routes exactly `/`,`/atlas/:plate`,`/people`,`/people/:id`,
...
- [ ] **U.3 — Plates + Atlas screens.**
  ...
  **Spec:** plate cards (natal withHouses/timeUnknown); Atlas views natal/synastry/today:
  ...
  applying/separating, 12-house-system chip selector (the §6 pinned set: P K O R C A V W T X B U),
  bi-wheel for synastry, no score/no label); honest-absence branches (J1/J3/J4/J5). Render
  tests assert cusp angles from fixture ChartFacts.
...
- [ ] **U.5 — Settings screen.**
  ...
  **Spec:** sections exactly per amended SCREEN.md: House system (12 chips: P K O R C A V W
  T X B U per ARCHITECTURE §6), Model catalogue from M.1, real download progress %, storage
  used, remove, trial-eligible flag + lock routing to /paywall), Voice (Kokoro picker,
  preview, mute — V.1/V.2 injection), License (status from B.1/B.3, Restore → B.5c, Enter a
  code → /paywall enter-code state), Data (export/import/deleted via X.1/X.2, Lore summary
  line `[turns · nodes]` from L.5 stats), About link; mobile + web (1280) layouts.
...
- [ ] **U.9 — Stage (mascot).**
  ...
  **Spec:** 8 states driven only by the C.4 `StageSignal` union (STATES.md real-vs-composed
  table): idle loop (webp) real; waking/thinking/speaking/delighted/error as composed
  overlays (SVG/glyph + envelope ring); asleep = no model; listening = composer focus;
  reduced-motion: frame 0 + no plate slide; envelope drives orb+mouth scale.
...
- [ ] **G.1 — Glossary + gazetteer.**
  ...
  **Spec:** GlossaryEntry per glyph (35: 12 signs, 11 bodies, 2 nodes, 7 aspects,
  retrograde, applying, separating — per TOKENS.md glyph decomposition and the
  `natally-glyphs.svg` symbol set) with `What it is` body (authored-static, INC-19) +
...
- [ ] **I.4 — Full workspace gate.** ⛓
  **Owns:** `scripts/assert-budget.mjs`; runs checks.
  **Spec:** `./scripts/check.sh` (T0.1) + `bash scripts/grep-no-speechsynthesis.sh` +
  bundle budget assert script (owned `scripts/assert-budget.mjs`: built web initial JS
  ≤ 300 KB gz).
...
- [ ] **R.6 — Version stamping wiring.** **Owns:** `scripts/update-version.sh`.
  **Spec:** add a read-only `--check` mode (exit 0 when every version surface agrees;
  nonzero otherwise; never writes — running it must not mutate the stamp). Migrate the
  manifest guards to the monorepo paths they silently miss today:
  `apps/local/src-tauri/tauri.conf.json`, `apps/local/src-tauri/Cargo.toml`,
  `apps/local/src-tauri/gen/android/tauri.properties` (the current guards point at the
  pre-monorepo root and no-op). Keep the default stamp behaviour and `--post-build` exactly
  as they are (house flow unchanged — no hardening).
