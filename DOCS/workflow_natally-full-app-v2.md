...
C.1 **port-verify-flip** (B: `inference-web.ts` + `lane.ts` + src-tauri `inference/`; native mount rides I.2) · V.2 **port-verify-flip** (B: `src/voice/**`; ban grep ships in the layer) · X.1 **port-verify-flip** (audit: 20/20) · X.2 **write** · B.3 **port-verify-flip** (billing token + tests) · **Mirror publish — Atomic Bots catalogue** (operator 2026-09-16) via verified HF token: manifest = **Qwen3.5‑2B Q4_K_M default + `trialEligible`** (compressed tiers UD‑Q3_K_XL/IQ4_XS same source; engine probe pending) · **LFM2.5‑2.6B GGUF** (`LiquidAI`) behind a **stability probe** (operator-recalled crash issues — verify before offering) · **Bonsai‑27B / Ternary‑Bonsai‑27B 1‑bit GGUF** (`prism-ml`) as the smart tier — slow load, offered only where GPU-fit is verified (Android precondition); Qwen3.5‑0.8B excluded from the offered list (below the 2B companion floor: DOM access + tool calling + lore decisions) · Kokoro + MiniLM embeddings; sha256 manifest throughout. U.5 renders the list (D11: exactly one trialEligible). Gate: companion speaks + reads aloud in dev PWA; data round-trips.

## Phase W3 — Screens from STITCH-v2 (B's screens demoted to wiring references)

Same 9 wiring tasks as v1 (U.1 align, U.8 splash, U.4 onboarding, U.2 conversation ×9 variants, U.9 sprite Stage — B's `stage.tsx`/`mascot.tsx` are the port base, U.3 atlas/torus, U.5 settings, U.7 about+glossary, U.6 paywall/checkout), each wired from `LIBS/UI/STITCH-v2/<screen>/code.html` element names verbatim. **Input law (operator 2026-09-16): every date entry is a structured, auto-expanded date picker — free-text date fields are forbidden; the fence's ambiguous-date absence ("use YYYY-MM-DD") remains an output-side backstop only, never a user-facing outcome.** Gate G3: all screens render; visual pass vs frozen renders.
