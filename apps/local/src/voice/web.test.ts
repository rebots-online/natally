...
import { createKokoroWebVoice, KOKORO_SAMPLE_RATE, KOKORO_WORKLET_SOURCE, type KokoroWebOptions } from "./web";
import { rmsEnvelope } from "./envelope";

const mocks = vi.hoisted(() => ({
  phonemize: vi.fn(), run: vi.fn(), release: vi.fn(), create: vi.fn(), adapter: vi.fn(),
  env: { wasm: { wasmPaths: "", numThreads: 0 } }, dispose: vi.fn(),
}));
...
const options: KokoroWebOptions = {
  modelUrl: "/models/kokoro.onnx", tokenizerUrl: "/models/tokenizer.json",
  voiceUrl: "/models/af_heart.bin", wasmPaths: "/ort/", language: "en-us",
};
const session = {
  inputNames: ["input_ids", "style", "speed"], outputNames: ["waveform"],
  run: mocks.run, release: mocks.release,
};
let emitted: unknown[];
...
class TestWorkletNode {
  ...
  connect = vi.fn();
  disconnect = vi.fn();
  constructor(public context: TestAudioContext, public name: string, public options: AudioWorkletNodeOptions) { nodes.push(this); }
  message(data: unknown) { this.port.onmessage?.({ data }); }
}

beforeEach(() => {
  vi.clearAllMocks();
  emitted = []; contexts = []; nodes = [];
  ...
  assets = new Map([
    ...
    ["/models/tokenizer.json", () => Response.json({ model: { vocab: { $: 0, a: 43, b: 44, " ": 16, ".": 4, ",": 3, "ɹ": 123, j: 52 } } })],
    ["/models/af_heart.bin", () => new Response(voice)],
  ]);
  vi.stubGlobal("fetch", vi.fn(async (url: string) => assets.get(new URL(url).pathname)?.() ?? new Response(null, { status: 404 })));
  ...
  mocks.run.mockReset().mockImplementation(async () => ({ waveform: { data: Float32Array.of(0.5, -0.5), dispose: mocks.dispose } }));
  mocks.release.mockReset().mockResolvedValue(undefined);
});
afterEach(() => { vi.restoreAllMocks(); vi.unstubAllGlobals(); });

async function voice(overrides: Partial<KokoroWebOptions> = {}) {
  return createKokoroWebVoice({ ...options, bus: { emit: (event) => { emitted.push(event); } }, ...overrides });
}
...
describe("Kokoro web inference and playback contract", () => {
  it("feeds phoneme IDs, real voice rows and speed, then emits playback-driven RMS", async () => {
    ...
    expect(mocks.create).toHaveBeenCalledWith(expect.any(ArrayBuffer), { executionProviders: ["wasm"] });
    ...
    await speaking;
    expect(emitted).toEqual([{ type: "envelope-start" }, { type: "envelope-level", level: 0.5 }, { type: "envelope-end" }]);
  ...
  it("tries WebGPU, then falls back to WASM on session initialization failure", async () => {
    ...
    const tts = await voice();
    expect(tts.backend).toBe("wasm");
    expect(mocks.create.mock.calls.map((call) => call[1])).toEqual([{ executionProviders: ["webgpu", "wasm"] }, { executionProviders: ["wasm"] }]);
    await tts.dispose();
  });
  ...
  it.each(["null", "rejected"])("reports WASM when no GPU adapter is available: %s", async (failure) => {
    vi.stubGlobal("navigator", { gpu: { requestAdapter: mocks.adapter } });
    ...
    const tts = await voice();
    expect(tts.backend).toBe("wasm");
    expect(mocks.create).toHaveBeenCalledExactlyOnceWith(expect.any(ArrayBuffer), { executionProviders: ["wasm"] });
    await tts.dispose();
  });
  ...
  it("retains punctuation and adapts eSpeak IPA to Kokoro's vocabulary", async () => {
    ...
    await finishPlayback(); await speaking;
    expect(mocks.phonemize.mock.calls).toEqual([["Hello", "en"], ["Robin", "en"]]);
    expect([...mocks.run.mock.calls[0][0].input_ids.data]).toEqual([0n, 123n, 52n, 3n, 16n, 123n, 52n, 4n, 0n]);
    await tts.dispose();
  });
  ...
  it("chunks more than 510 phonemes without silently dropping the remainder", async () => {
    ...
    await finishPlayback(); await speaking;
    expect(mocks.run.mock.calls.map((call) => call[0].input_ids.dims)).toEqual([[1, 512], [1, 512], [1, 3]]);
  ...
  it("does not emit a level after an envelope-start listener stops playback", async () => {
    const tts = await voice({ bus: { emit: (event) => {
      emitted.push(event);
  ...
  it("stops active playback once and rejects overlapping calls", async () => {
    ...
    tts.stop(); tts.stop();
    await rejected;
    expect(emitted.filter((event: any) => event.type === "envelope-end")).toHaveLength(1);
    expect(emitted.some((event: any) => event.type === "error")).toBe(false);
    await tts.dispose(); await tts.dispose();
    expect(mocks.release).toHaveBeenCalledOnce();
  ...
  it("accepts caller cancellation and no-ops empty input", async () => {
    const tts = await voice();
    await tts.speak("  ");
    await expect(tts.speak("Hello", { signal: AbortSignal.abort() })).rejects.toMatchObject({ name: "AbortError" });
  ...
  it.each(["processor", "suspend"])("ends the envelope when playback fails: %s", async (failure) => {
    const tts = await voice();
    const speaking = tts.speak("Hello");
    ...
    else { contexts[0].state = "suspended"; contexts[0].dispatchEvent(new Event("statechange")); }
    await rejected;
    expect(emitted.slice(-2)).toEqual([{ type: "envelope-end" }, { type: "error", message: expect.any(String) }]);
    await tts.dispose();
  });
  ...
  it("rejects invalid model output", async () => {
    mocks.run.mockResolvedValueOnce({ waveform: { data: Float32Array.of(NaN), dispose: mocks.dispose } });
    const tts = await voice();
  ...
  it("rejects unavailable and malformed assets without creating a session", async () => {
    ...
    await expect(voice({ modelUrl: "https://elsewhere.test/model.onnx" })).rejects.toThrow("origin");
  ...
  it("reads model, tokenizer and voice through the verified cache adapter while offline", async () => {
    ...
    const readAsset = vi.fn(async (url: string) => assets.get(new URL(url).pathname)!());
  ...
  it("retains response, tokenizer, and voice validation for cached assets", async () => {
    const readAsset = async (url: string) => assets.get(new URL(url).pathname)!();
    ...
    assets.set("/models/tokenizer.json", () => Response.json({ model: { vocab: { $: 0, a: 43 } } }));
...
describe("production AudioWorklet processor", () => {
  ...
  function createProcessor(pcm: Float32Array, rate: number, onMessage: (message: Message) => void) {
    let Implementation!: new (options: { processorOptions: { pcm: Float32Array; sourceRate: number } }) => Processor;
    runInNewContext(KOKORO_WORKLET_SOURCE, {
      sampleRate: rate,
      AudioWorkletProcessor: class { port = { postMessage: onMessage }; },
      registerProcessor: (_name: string, value: typeof Implementation) => { Implementation = value; },
    });
  ...
  it("resamples 24 kHz PCM to 48 kHz and reports exactly the rendered frames", () => {
    ...
    expect(events.filter((event) => event.type === "frame").flatMap((event) => [...event.samples])).toEqual(output);
  ...
  it.each([24_000, 44_100, 48_000, 96_000])("emits 20 ms RMS windows at %i Hz across changing render quanta", async (rate) => {
    // 50 ms from Kokoro: two full envelope windows plus a 10 ms final window.
    mocks.run.mockResolvedValueOnce({ waveform: { data: new Float32Array(1200).fill(0.5), dispose: mocks.dispose } });
    const tts = await voice();
    const speaking = tts.speak("Hello");
    await vi.waitFor(() => expect(nodes).toHaveLength(1));
    ...
    let iteration = 0;
    let running = true;
    const total = rate * 0.050;
    const windowSize = rate * 0.020;
    while (running) {
      ...
      const expected = Math.floor(processed / windowSize) + (!running && processed % windowSize !== 0 ? 1 : 0);
    ...
    await speaking;
    expect(windows.map((window) => window.length)).toEqual([windowSize, windowSize, rate * 0.010]);
    expect(emitted).toEqual([
      { type: "envelope-start" },
