...
function mergeById<T extends { id: string; ts: number }>(older: readonly T[], newer: readonly T[]): T[] {
...
export function useConversation(services: ConversationServices, sessionId: string) {
  ...
  const stage = useSyncExternalStore(bus.stageState$.subscribe, bus.stageState$.getSnapshot, bus.stageState$.getSnapshot);
  ...
  useEffect(() => {
    ...
    return () => { active = false; stop(); if (frame !== undefined) cancelAnimationFrame(frame); };
  }, [bus]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = [
      ...
      bus.subscribe("chart-computed", ({ chartId }) => {
        ...
        void Promise.resolve().then(() => services.plates!.get(chartId)).then((plate) => {
          ...
          if (active && plateRequests.get(chartId) === revision) setPlateError(errorReason(error));
        });
      }),
    ...
    void Promise.resolve().then(() => services.turns.list(sessionId)).then((history) => {
      if (active) setTurns((previous) => mergeById(history.filter((turn) => turn.sessionId === sessionId), mergeById(previous, liveTurns)));
    }).catch((error: unknown) => {
      if (active) setHistoryError(errorReason(error));
    }).finally(() => { if (active) setLoading(false); });
    if (services.plates) {
      void Promise.resolve().then(() => services.plates!.list(sessionId)).then((history) => {
        if (active) setPlates((previous) => mergeById(history.filter((plate) => plate.sessionId === sessionId), mergeById(previous, livePlates)));
      }).catch((error: unknown) => { if (active) setPlateError(errorReason(error)); });
    }
    return () => {
      active = false;
  ...
  return { stage, gate, modelPresent, turns, plates, stream, loading, historyError, plateError, engineError, reloadHistory: () => setReload((value) => value + 1) };
}
