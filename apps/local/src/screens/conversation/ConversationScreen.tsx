...
export function ConversationContextChip({
  ...
  return (
    <Chip active onClick={onClick} disabled={!onClick} aria-label="Conversation context">
      {gate && gate.state !== "licensed" ? "Trial" : label}
    </Chip>
  );
}

function GeneratedTurn({
  children,
  streaming = false,
}: {
  children: ReactNode;
  ...
  return (
    <article
      className="conversation__her"
      data-provenance="generated"
      aria-label="natally's message"
      aria-busy={streaming}
    >
      <Glyph name="sun" className="conversation__glyph" />
      <div>
        <div className="conversation__name">natally</div>
        <p>{children}</p>
      </div>
    </article>
  );
...
function Content({ content }: { content: ConversationContent }) {
  if (content.kind === "authored-static")
    return (
      <div data-provenance="authored-static">
        <strong className="conversation__education-label">{content.label}</strong>
        <p>{content.text}</p>
      </div>
    );
  return (
    <p
      data-provenance={content.kind}
      className={content.kind === "absence" ? "conversation__aside" : undefined}
    >
      {content.text}
    </p>
  );
...
function TrialEnded({ policy }: { policy?: TrialPolicy }) {
  ...
  return policy.readings === 3 ? (
    <>Your three trial readings are used.</>
  ) : (
    <>
      Your{" "}
      <span className="conversation__data" data-provenance="computed">
        {policy.readings}
      </span>{" "}
      trial readings are used.
    </>
  );
}
...
function ConversationSession(props: ConversationScreenProps) {
  ...
  const disabled =
    ...
    !!state.stream;
  useEffect(() => {
    if (disabled && bus.getSnapshot().composerFocused)
      bus.signal({ type: "composer-focus", focused: false });
  }, [disabled, bus]);
  ...
  async function runAction(action: (() => void | Promise<void>) | undefined, name: string) {
    ...
    if (!action || runningAction.current || !signal) return;
  ...
  const stage = (
    <Stage
      bus={bus}
      className={`conversation__stage${collapsed ? " conversation__stage--avatar" : ""}`}
    />
  );
  const context = (
    <ConversationContextChip gate={gate} label={props.contextLabel} onClick={props.onContext} />
  );
  const items = [
    ...state.turns
      .filter((turn) => turn.role !== "tool")
      .map((turn) => ({ kind: "turn" as const, id: turn.id, ts: turn.ts, turn })),
  ...
  return (
    <section
      ...
      data-rail={props.peopleAndSessions != null}
      data-atlas={props.atlas != null}
    >
      {props.peopleAndSessions != null && (
        <aside className="conversation__rail" aria-label="People and sessions">
          {props.peopleAndSessions}
        </aside>
      )}
      <div className="conversation__column">
        <div className="conversation__header">
          {props.renderHeader ? (
            props.renderHeader(context, collapsed ? stage : null)
          ) : (
            <>
              {collapsed && stage}
              {context}
        ...
        >
          ...
          <div
            className="conversation__transcript"
            role="log"
          ...
          >
            {state.stage === "Asleep" && (
              <section
                className="conversation__absence-plate"
                data-provenance="absence"
                aria-label="Wake natally"
              >
                <h2>natally is asleep</h2>
                <p>
                  She thinks with a language model that runs on this device. Download one to wake
                  her.
                </p>
                {props.model ? (
                  <div className="conversation__model">
                    <span className="conversation__data" data-provenance="computed">
                      {[
                        props.model.label,
                        props.model.quant,
                        props.model.bytes === undefined
                          ? null
                          : `${props.model.bytes.toLocaleString()} bytes`,
                      ]
                    ...
                    <Button
                      ...
                      disabled={!props.onDownloadModel || activeAction !== null}
                      onClick={() =>
                        void runAction(() => {
                          const model = props.model;
                          if (model) return props.onDownloadModel?.(model.id);
                        }, "download")
                      }
                    >
                  ...
                  <p className="conversation__aside">
                    No model is selected. Choose one in Settings.
                  </p>
                )}
                <p className="conversation__aside">
                  Charts work without her. Pick a different model in Settings.
                </p>
              </section>
            )}
            {state.engineError !== null && (
              <section
                className="conversation__absence-plate conversation__absence-plate--error"
                data-provenance="absence"
                role="alert"
              >
                <h2>natally can’t respond right now</h2>
                <p className="conversation__aside">{state.engineError}</p>
                <Button
                  disabled={!props.onRetry || activeAction !== null}
                  onClick={() => void runAction(props.onRetry, "retry")}
                >
                  Try again
                </Button>
              </section>
            ...
            {state.loading && (
              <p className="conversation__aside" data-provenance="absence" role="status">
            ...
            {state.historyError !== null && (
              <div
                ...
                role="alert"
              >
                <p className="conversation__aside">{state.historyError}</p>
                <Button onClick={state.reloadHistory}>Try loading again</Button>
              </div>
            )}
            {props.content?.map((content) => (
              <Content key={`${content.kind}:${content.text.slice(0, 48)}`} content={content} />
            ))}
            {items.map((item) =>
              item.kind === "turn" ? (
                <div key={`turn:${item.id}`} data-turn-id={item.id}>
                  {item.turn.role === "you" ? (
                    <TurnYou>{item.turn.text}</TurnYou>
                  ) : (
                    <GeneratedTurn>{item.turn.text}</GeneratedTurn>
                  )}
                </div>
              ) : (
                <div key={`plate:${item.id}`} data-plate-id={item.id}>
                  <PlateCard
                    title={item.plate.title}
                    provenance={<span data-provenance="computed">{item.plate.provenance}</span>}
                    onOpen={() => props.onOpenPlate(item.id)}
                  >
                    {item.plate.body}
                    <div className="conversation__content">
                      {item.plate.content.map((content) => (
                        <Content
                          key={`${content.kind}:${content.text.slice(0, 48)}`}
                          content={content}
                        />
                      ))}
                    </div>
            ...
            {variant === "Thinking" && (
              <p className="conversation__aside" data-provenance="absence" role="status">
                {state.stream ? "Responding…" : "Thinking…"}
              </p>
            )}
            {state.plateError !== null && (
              <p className="conversation__aside" data-provenance="absence" role="alert">
                {state.plateError}
              </p>
            )}
            {!state.loading &&
              ...
              !props.content?.length && (
                <p className="conversation__aside" data-provenance="absence">
                  No conversation yet.
                </p>
              )}
          </div>
        ...
        <footer className="conversation__footer">
          ...
          {gate?.state === "trial-exhausted" ? (
            <div className="conversation__locked">
              <p className="conversation__aside" data-provenance="absence">
                <TrialEnded policy={props.trialPolicy} />
              </p>
              <Button variant="primary" onClick={props.onUnlock}>
                Unlock natally
              </Button>
              <Button variant="quiet" onClick={props.onEnterCode}>
                or enter a code
              </Button>
            </div>
          ) : (
            <>
              {gate?.state === "rate-limited" && (
                <p className="conversation__aside" data-provenance="absence">
                  Your next reading unlocks{" "}
                  <time
                    className="conversation__data"
                    data-provenance="computed"
                    dateTime={new Date(gate.nextReadingAt).toISOString()}
                  >
                    {new Date(gate.nextReadingAt).toLocaleString()}
                  </time>
                  .
                </p>
              )}
              ...
              {state.modelPresent === null && (
                <p className="conversation__aside" data-provenance="absence">
                  Companion availability is not known yet.
                </p>
              )}
              <Composer
                value={draft}
                ...
                label="Ask natally"
                onAttach={
                  props.onAttach ? () => void runAction(props.onAttach, "attach") : undefined
                }
                onVoice={props.onVoice ? () => void runAction(props.onVoice, "voice") : undefined}
                voiceActive={props.voiceActive}
                onFocus={() => bus.signal({ type: "composer-focus", focused: true })}
                onBlur={() => bus.signal({ type: "composer-focus", focused: false })}
              />
            </>
          )}
        </footer>
      ...
      {props.atlas != null && (
        <aside className="conversation__atlas" aria-label="Atlas">
          {props.atlas}
        </aside>
      )}
    </section>
