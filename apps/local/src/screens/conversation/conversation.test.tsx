...
const reply: Turn = {
  ...
  sessionId,
  role: "her",
  text: "Published response from the scripted service.",
  ts: 102,
};
const plate: ConversationPlate = {
  ...
  sessionId,
  ts: 101,
  title: "Natal · Fixture person",
  provenance: "Placidus · fixture inputs",
  content: [
...
beforeEach(() => {
  ...
  vi.stubGlobal("matchMedia", (query: string) => ({
    media: query,
    get matches() {
      return query === "(min-width: 960px)" ? wide : true;
    },
...
function harness(
  initial: GateResult | null = { state: "licensed" },
  present: boolean | null = true,
) {
...
function variant() {
  return screen
    ...
    .getAttribute("data-conversation-variant");
}
function stageState() {
  return screen.getByRole("region", { name: "Companion stage" }).getAttribute("data-stage-state");
}
function field() {
  return screen.getByRole<HTMLTextAreaElement>("textbox", { name: "Ask natally" });
}
...
describe("conversation: 9 variants render against event scripts", () => {
  it("Idle: renders accepted turns, real plate data and provenance with the frozen transcript grammar", async () => {
    ...
    h.send(
      { type: "turn", turn: userTurn },
      { type: "turn", turn: reply },
      { type: "chart-computed", chartId: plate.id },
    );
    ...
    expect(
      generated.querySelector(".conversation__glyph svg, svg.conversation__glyph"),
    ).not.toBeNull();
    ...
    expect(screen.getByText("What it is").parentElement?.getAttribute("data-provenance")).toBe(
      "authored-static",
    );
    ...
    expect(
      [...container.querySelectorAll("[data-turn-id], [data-plate-id]")].map(
        (node) => node.getAttribute("data-turn-id") ?? node.getAttribute("data-plate-id"),
      ),
    ).toEqual([userTurn.id, plate.id, reply.id]);
  });
  ...
  it("Thinking: shows no reply before produced tokens, then replaces the stream with the published turn", async () => {
    ...
    expect(stageState()).toBe("Thinking");
    ...
    expect(
      screen.getByRole("article", { name: "natally's message" }).getAttribute("aria-busy"),
    ).toBe("true");
  ...
  it("Speaking: only audio envelope events activate speaking on the real Stage", async () => {
    ...
    expect(variant()).toBe("Speaking");
    expect(stageState()).toBe("Speaking");
  ...
  it("Asleep: uses observed no-model capability and injected manifest/download without inventing wake success", async () => {
    ...
    expect(stageState()).toBe("Asleep");
    h.send({ type: "engine-load", fraction: 0.3 });
    expect(stageState()).toBe("Waking");
    expect(field().disabled).toBe(true);
    h.send({ type: "engine-load", fraction: 1 });
    expect(variant()).toBe("Idle");
  });
  ...
  it("Error: preserves repeated real reasons and waits for runtime recovery after Try again", async () => {
    ...
    expect(container.querySelector(".conversation__absence-plate--error")?.textContent).toContain(
      "missing weights",
    );
  ...
  it("TrialExhausted: replaces the composer, retains history, and delegates unlock and code actions", async () => {
    ...
    expect(h.props.onUnlock).toHaveBeenCalledOnce();
    expect(h.props.onEnterCode).toHaveBeenCalledOnce();
  ...
  it("RateLimited: displays the actual runtime date, disables sending, and does not unlock on its own timer", async () => {
    ...
    expect(container.querySelector("time")?.textContent).toBe(
      new Date(nextReadingAt).toLocaleString(),
    );
  ...
  it("Desktop: composes the supplied People/Sessions and Atlas panels without another router or Stage", async () => {
    ...
    render(
      <ConversationScreen
        {...h.props}
        peopleAndSessions={<p>Persisted people and sessions</p>}
        atlas={<p>Opened computed atlas</p>}
      />,
    );
    await settled();
    act(() => {
    ...
    expect(
      screen.getByRole("complementary", { name: "People and sessions" }).textContent,
    ).toContain("Persisted");
    expect(screen.getByRole("complementary", { name: "Atlas" }).textContent).toContain(
      "computed atlas",
    );
...
describe("conversation integration and truthful rendering", () => {
  it("submits once through the injected service, observes focus, and renders only published events", async () => {
    ...
    expect(stageState()).toBe("Listening");
    ...
    expect(h.services.submit).toHaveBeenCalledWith(userTurn.text, {
      sessionId,
  ...
  it("restores history without re-emitting Stage events and lets live rows win the hydration race", async () => {
    ...
    await act(async () => {
      history.resolve([
        userTurn,
        { ...reply, text: "Older row" },
        { ...reply, id: "foreign", sessionId: "other" },
      ]);
  ...
  it("shows storage failures and retries history while preserving observed messages", async () => {
    const h = harness();
    h.services.turns.list = vi
      .fn()
      .mockRejectedValueOnce(new Error("SQLite read failed: busy"))
  ...
  it("cancels in-flight submission on session switch and ignores late history, plates and old bus events", async () => {
    ...
    const { rerender } = render(<ConversationScreen {...h.props} />);
    await settled();
    ...
    rerender(<ConversationScreen {...next.props} sessionId="next-session" />);
    await settled();
    ...
    await act(async () => {
      ...
      resolvingPlate.resolve(plate);
      submitted.resolve();
      await Promise.all([history.promise, resolvingPlate.promise, submitted.promise]);
    });
  ...
  it("does not mislabel arbitrary tool text as computed facts or generated interpretation", async () => {
    ...
    render(
      <ConversationScreen
        {...h.props}
        content={[
          { kind: "computed", text: "Moon 27°51′" },
          { kind: "absence", text: "No house positions without birth time." },
        ]}
      />,
    );
    await settled();
    ...
    expect(
      screen.getByText("No house positions without birth time.").getAttribute("data-provenance"),
    ).toBe("absence");
  });
  ...
  it("does not claim a three-reading count for a time trial", async () => {
    ...
    render(
      <ConversationScreen
        {...h.props}
        trialPolicy={{ mode: "time", days: 7, trialModel: "manifest-model" }}
      />,
    );
    await settled();
  ...
  it("keeps unresolved access and model capability honest, including a same-state model observation", async () => {
    ...
    const { unmount } = render(<ConversationScreen {...h.props} />);
    await settled();
    expect(field().disabled).toBe(true);
    ...
    h.send({ type: "model-presence", present: true });
    frame();
  ...
  it("disables unavailable optional controls and delegates supplied voice/attachment actions", async () => {
    ...
    expect(screen.getByRole("button", { name: "Voice input" }).getAttribute("aria-pressed")).toBe(
      "true",
    );
  ...
  it("reports plate lookup absence/failure without a made-up wheel", async () => {
    const h = harness();
    h.services.plates!.get = vi
      .fn()
      .mockResolvedValueOnce(null)
      .mockRejectedValueOnce(new Error("Chart read failed"));
    render(<ConversationScreen {...h.props} />);
    await settled();
  ...
  it("exports the context chip for shell integration and reads all frozen variant names", () => {
    render(
      <ConversationContextChip gate={{ state: "trial-active", remaining: 1 }} onClick={vi.fn()} />,
    );
    expect(screen.getByRole("button", { name: "Conversation context" }).textContent).toBe("Trial");
    const ledger = frozenLedger as unknown as { screens: Record<string, Record<string, unknown>> };
    expect(Object.keys(ledger.screens["screen-conversation"]).sort()).toEqual(
      [
        "Idle",
        "Thinking",
        "Speaking",
